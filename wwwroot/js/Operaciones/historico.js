...
App.historico = (function () {
    ...
    function init(historicoData, usuariorol, asesores, supervisores) {
        ...
        console.log('Inicializando histórico con datos:', historico); // --- IGNORE ---
        console.log('Rol de usuario:', rol);
        console.log('Asesores de usuario:', usuarioAsesores);
        console.log('Supervisores de usuario:', usuarioSupervisores);

        const gridDiv = document.getElementById('gridHistóricoReagendamientos');
        if (!gridDiv) {
            console.error('No se encontró el elemento con ID "gridHistóricoReagendamientos"');
            return;
        }
        derivacionesGridOptions.rowData = historico;
