...
WHERE CAST(fecha_subida AS DATE) = '2025-08-15'
...
WHERE 
    CAST(bcb_destino.fecha_subida AS DATE) = '2025-08-15'
    AND (
...
select COUNT(*) from base_clientes_banco WHERE CAST(fecha_subida as date) = '2025-08-15'
AND (PATERNO IS NULL OR NOMBRES IS NULL OR MATERNO IS NULL)

select dni from base_clientes_banco WHERE CAST(fecha_subida as date) = '2025-08-15'
AND (PATERNO IS NULL OR NOMBRES IS NULL OR MATERNO IS NULL)
