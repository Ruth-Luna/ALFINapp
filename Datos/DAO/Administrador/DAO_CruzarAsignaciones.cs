...
{
    public class DAO_CruzarAsignaciones
    {
        private DA_Usuario _dausuario = new DA_Usuario();
        private readonly MDbContext _context;
        public DAO_CruzarAsignaciones(MDbContext context)
        {
            _context = context;
        }
        public async Task<(bool IsSuccess, string Message, DetallesAssignmentsMasive? ClientesCruzados)> cruzar(DetallesAssignmentsMasive clientes)
        {
            try
            {
                ...
                var cross = await CrossAssignments(clientes);
                if (!cross.IsSuccess)
                {
                ...
                foreach (var supervisor in clientes.SupervisoresConClientes)
                {
                    ...
                    var listed = await CreateListName(supervisor.DniSupervisor);
                    if (!listed.IsSuccess)
                    {
        ...
        public async Task<(bool IsSuccess, string Message, string NombreLista)> CreateListName(string dni_supervisor)
        {
            try
            {
                var getSupervisor = _dausuario.getUsuarioPorDni(dni_supervisor);
                ...
                var existingList = await _context.listas_asignacion
                    .Where(l => l.IdUsuarioSup == getSupervisor.IdUsuario)
                    .Where(l => l.FechaCreacion != null)
                    .OrderByDescending(l => l.FechaCreacion != null ? l.FechaCreacion.Value : DateTime.MinValue)
                    .FirstOrDefaultAsync();
