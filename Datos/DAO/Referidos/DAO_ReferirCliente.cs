...
{
    public class DAO_ReferirCliente
    {
        private readonly MDbContext _context;
        public DAO_ReferirCliente(MDbContext context)
        {
            _context = context;
        }
        public async Task<(bool isSuccess, string Message)> ReferirClienteAsync(Cliente cliente, Vendedor asesor)
        {
            try
            {
                if (cliente == null)
                ...
                var result = await ReferirCliente(cliente, asesor);
                if (!result.IsSuccess)
                {
                    return (false, result.Message);
                }

                var emailResult = await EnviarCorreoReferido(cliente.Dni ?? string.Empty);
                if (!emailResult.IsSuccess)
                {
                    return (false, emailResult.Message);
                }

                return (true, "Cliente referido y correo enviado correctamente.");
            }
            catch (Exception ex)
            {
        ...
        public async Task<(bool IsSuccess, string Message, List<ViewReferidos> Data)> GetClientesReferidos(string dni_asesor)
        {
            try
            {
                var parameters = new SqlParameter[]
                {
                    new SqlParameter("@dni", dni_asesor ?? (object)DBNull.Value)
                };
                var result = await _context.referidos_get_referidos_por_dni.FromSqlRaw(
                    "EXEC dbo.SP_REFERIDOS_GET_REFERIDOS_POR_DNI @DNI",
                    parameters
                ).ToListAsync();

                if (result == null || result.Count == 0)
                {
                    return (false, "No se encontraron clientes referidos para el DNI proporcionado.", new List<ViewReferidos>());
                }
                var referidos = result.Select(r => new ViewReferidos(r)).ToList();
                if (referidos.Count == 0)
                {
                    return (false, "No se encontraron clientes referidos para el DNI proporcionado.", new List<ViewReferidos>());
                }
                return (true, "Clientes referidos obtenidos correctamente.", referidos);
            }
            catch (System.Exception ex)
            {
